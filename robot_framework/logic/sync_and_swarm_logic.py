...
class SyncAndSwarmPhaseLogic:
    ...
    def update_phase(self, state, positions, phases):
        ...
        if N == 0:
            ...
            external_change = float(self.K) / N * np.sum(
                np.array([
        ...
        if self.oscillations_on:
            ...
            natural_increase = 0
        return (
            natural_increase + phase + external_change
        ) % (2 * np.pi) / (2 * np.pi)
...
class SyncAndSwarmPositionLogic:
    ...
    def update_position(self, state, positions, phases):
        ...
        if self.align_center:
            ...
            center_alignment = np.zeros(3)
        vel = external_change + center_alignment
